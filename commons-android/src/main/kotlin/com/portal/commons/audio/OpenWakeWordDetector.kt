...
class OpenWakeWordDetector private constructor(
    ...
    override fun updateWakeWords(words: List<WakeWord>) {
        ...
        if (configs.isEmpty()) {
            events.onUnavailable(NAME)
            return
        }
        if (!installClassifiers(configs)) {
            events.onDiagnostic(NAME, "oww: bundled phrase model swap failed — keeping previous models")
        }
    }
    ...
    fun updatePhraseModels(configs: List<PhraseClassifierConfig>) {
        ...
        if (!installClassifiers(configs)) {
            events.onDiagnostic(NAME, "oww: phrase model swap failed — keeping previous models")
        }
    }
    ...
    private fun step() {
        ...
        for (classifier in loadedClassifiers) {
            ...
            if (score >= classifier.scoreThreshold && classifier.stepsSinceFire >= REFRACTORY_STEPS) {
                ...
                events.onWake(NAME, classifier.wakeId, "score=${"%.3f".format(score)}")
            }
        }
    }
     ...
     * [WakeWord.scoreThreshold] / `com.portal.wake.min_confidence`).
     */
    ...
    companion object {
        ...
        fun buildBundledPhraseConfigs(context: Context, words: List<WakeWord>): List<PhraseClassifierConfig> =
            buildList {
                for (word in words) {
        ...
        fun factory(phraseConfigs: List<PhraseClassifierConfig>): WakeDetector.Factory =
            WakeDetector.Factory { context, _, events ->
                OpenWakeWordDetector(context, ModelSource.EXPLICIT, phraseConfigs, emptyList(), events)
            }
        ...
        fun factory(): WakeDetector.Factory =
            WakeDetector.Factory { context, words, events ->
                OpenWakeWordDetector(context, ModelSource.BUNDLED, emptyList(), words, events)
            }
